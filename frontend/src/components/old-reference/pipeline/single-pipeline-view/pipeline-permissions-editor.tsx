"use client";
import { Pipeline, PipelinePermissionsSchema } from "@/definitions";
import {
  Button,
  CardList,
  Dialog,
...
import { useUpdatePipelinePermissions } from "@/data/use-pipeline";
...
export default function PipelinePermissionsEditor({
  pipeline,
  ...
  const { trigger: updatePermissions, isMutating } =
    useUpdatePipelinePermissions(pipeline.id);
  ...
  function isValidPermissionsUpdate() {
    ...
    const result = PipelinePermissionsSchema.safeParse({
      read: readUsers,
      update: updateUsers,
      delete: deleteUsers,
    });
  ...
  async function updatePipelinePermissions() {
    ...
    const parsedPermissions =
      PipelinePermissionsSchema.parse(pendingPermissions);
  ...
  function renderDialogTitle() {
    return (
      <div className="flex flex-row items-center">
        <H6 className="w-fit">Manage pipeline permissions</H6>
        {!isMutating && (
          <Tabs
            animate
              ...
              if (isAlreadyAdded || isCreator) {
                return (
                  <UserPermissionsCard
                    ...
                    rightElement={
                      <Tag>
                        {isCreator ? "Pipeline creator" : "User already added"}
                      </Tag>
                    }
                  />
                );
        ...
        <DialogFooter
          actions={
            <Button
              ...
              onClick={() => updatePipelinePermissions()}
            />
          }
        />
      );
