...
export default function InvalidStepPopover({ errors }: { errors: any[] }) {
  return (
    <Popover
      content={
        <div className="p-1">
          <Callout
            intent="warning"
            icon="warning-sign"
            title="This display includes invalid steps"
          >
      ...
      placement="right"
    >
      <div className="flex flex-row items-center p-1 ml-2 border rounded-sm cursor-pointer border-error">
        <Icon className="mr-1" icon="warning-sign" color={Colors.ORANGE3} />
        <Text className="font-bold">Invalid steps</Text>
      </div>
    </Popover>
  );
}
